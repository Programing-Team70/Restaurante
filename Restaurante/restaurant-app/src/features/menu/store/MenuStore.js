...
export const useMenuStore = create((set, get) => ({
  ...
  getMenus: async (id) => {
    try {
      ...
      if (id) {
        set({
          menus: response.data?.item ? [response.data.item] : [],
          loading: false,
        });
      } else {
    ...
    try {
      ...
      set({
        error:
          ...
          "Error al obtener los platillos del restaurante.",
        loading: false,
      });
    }
  },
  ...
  updateMenu: async (id, menuData) => {
    try {
      ...
      set({
        error:
          error.response?.data?.message || "Error al actualizar el platillo.",
        loading: false,
      });
    }
  },
  ...
  deleteMenu: async (id) => {
    try {
      ...
      set({
        menus: get().menus.filter((menu) => menu._id !== id && menu.id !== id),
        loading: false,
      });
    } catch (error) {
      set({
        error:
          error.response?.data?.message || "Error al desactivar el platillo.",
        loading: false,
      });
    }
  },
