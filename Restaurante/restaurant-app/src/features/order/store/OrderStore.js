    ...
    try {
      ...
      set({
        error:
          ...
          "Error al obtener los pedidos en Heaven Flavor.",
        loading: false,
      });
    }
  },
  ...
  getOrdersByRestaurant: async (restaurantId) => {
    try {
      ...
      set({
        error:
          error.response?.data?.message ||
          "Error al obtener los pedidos del restaurante.",
        loading: false,
      });
    }
  },
    ...
    try {
      ...
      set({
        orders: get().orders.filter(
          (order) => order._id !== id && order.id !== id,
        ),
        loading: false,
      });
    } catch (error) {
      set({
        error:
          error.response?.data?.message ||
          "Error al eliminar el pedido del sistema.",
        loading: false,
      });
    }
  },
