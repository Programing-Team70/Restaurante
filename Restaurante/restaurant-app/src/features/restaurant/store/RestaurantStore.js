...
export const useRestaurantStore = create((set, get) => ({
  ...
  getRestaurants: async () => {
    try {
      ...
      set({
        error:
          error.response?.data?.message || "Error al obtener los restaurantes",
        loading: false,
      });
    }
  },
  ...
  createRestaurant: async (formData) => {
    try {
      ...
      set({
        error:
          error.response?.data?.message || "Error al crear el restaurante.",
        loading: false,
      });
      throw error;
    }
    ...
    try {
      ...
      set({
        restaurants: get().restaurants.filter(
          (restaurant) => restaurant._id !== id && restaurant.id !== id,
        ),
        loading: false,
      });
    } catch (error) {
      set({
        error:
          error.response?.data?.message ||
          "Error al desactivar el restaurante.",
        loading: false,
      });
    }
  },
