...
import { XCircle, Layers, DollarSign, Tag, CheckCircle2, Edit3, Trash2 } from 'lucide-react';

export const MenuContainerModal = ({ isOpen, onClose, selectedPlate, onEditClick, onDeleteClick }) => {
    if (!isOpen || !selectedPlate) return null;

    const handleDelete = () => {
        if (window.confirm(`¿Está seguro de que desea eliminar "${selectedPlate.name}" de Heaven Flavor?`)) {
            onDeleteClick(selectedPlate);
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 w-full max-w-xl overflow-hidden transform transition-all z-10 animate-in zoom-in-95 duration-200">
                <div className="bg-[#0a192f] text-white px-8 py-5 flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-xl font-bold tracking-wide">{selectedPlate.name}</h3>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div className="p-8 space-y-6 text-sm text-left">
                    <div className="space-y-1.5">
                        ...
                        <p className="text-gray-600 leading-relaxed bg-gray-50 p-4 rounded-xl border border-gray-100">
                            {selectedPlate.description || "Este platillo no cuenta con una descripción."}
                        </p>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {Array.isArray(selectedPlate.ingredients) && selectedPlate.ingredients.length > 0 ? (
                                selectedPlate.ingredients.map((ing, index) => (
                                    <span 
                                        key={index}
                                        className="bg-gray-100 text-gray-700 text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200/60"
                                    >
                                        {ing}
                                    </span>
                                ))
                            ) : (
                                <p className="text-xs text-gray-400 italic">No se especificaron ingredientes para esta receta.</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-gray-100 flex items-center justify-between gap-4">
                        <div className="flex flex-col gap-0.5 text-[10px] text-gray-400">
                            <p><strong>ID Menú:</strong> {String(selectedPlate._id || selectedPlate.id || 'N/A')}</p>
                            <p><strong>ID Relación:</strong> {String(selectedPlate.restaurant || 'Global')}</p>
                        </div>
